...
export async function POST(request: Request) {
  try {
    ...
    const [student] = await db.select({ id: students.id, studentId: students.studentId, name: students.name }).from(students).where(and(eq(students.id, Number(studentId)), eq(students.classId, classId))).limit(1);
