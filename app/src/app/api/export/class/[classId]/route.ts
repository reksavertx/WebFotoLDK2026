...
export async function GET(request: Request, context: { params: Promise<{ classId: string }> }) {
  ...
  const rows = await db.select({ studentId: students.studentId, name: students.name, attendance: students.attendanceNumber, photoPath: photos.storagePath }).from(students).leftJoin(photos, eq(photos.studentId, students.id)).where(eq(students.classId, classId)).orderBy(asc(students.attendanceNumber));
  const data = rows.map((row) => ({ student_id: row.studentId, name: row.name, attendance: row.attendance, class: classRow.name, photo_url: row.photoPath ? `/api/photos/${row.studentId}` : null }));
