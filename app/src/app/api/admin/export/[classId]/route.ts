...
export async function GET(_: Request, context: { params: Promise<{ classId: string }> }) {
  ...
  if (classIdParam === "all") {
    ...
    for (const classRow of classRows) {
      ...
      const rows = await db.select({ name: students.name, attendance: students.attendanceNumber, storagePath: photoSubmissions.storagePath, mimeType: photoSubmissions.mimeType }).from(students).leftJoin(photoSubmissions, eq(photoSubmissions.studentId, students.id)).where(eq(students.classId, classRow.id)).orderBy(asc(students.attendanceNumber));
  ...
  const rows = await db.select({ name: students.name, attendance: students.attendanceNumber, storagePath: photoSubmissions.storagePath, mimeType: photoSubmissions.mimeType }).from(students).leftJoin(photoSubmissions, eq(photoSubmissions.studentId, students.id)).where(eq(students.classId, classId)).orderBy(asc(students.attendanceNumber));
  for (const row of rows) if (row.storagePath) {
