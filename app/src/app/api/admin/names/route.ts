...
export async function GET(request: Request) {
  ...
  const rows = await db.select({ name: students.name, attendance: students.attendanceNumber }).from(students).innerJoin(classes, eq(classes.id, students.classId)).leftJoin(photoSubmissions, eq(photoSubmissions.studentId, students.id)).where(status === "pending" ? isNull(photoSubmissions.id) : eq(photoSubmissions.status, status)).orderBy(asc(students.classId), asc(students.attendanceNumber));
